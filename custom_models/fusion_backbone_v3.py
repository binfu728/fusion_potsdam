        ...
        *,
        ...
        if has_fusion:
            ...
            if olmoearth is not None:
                # olmoearth: (B, H_emb, W_emb, 768) from .npy embedding file
                # Spatial pool from embedding resolution (e.g. 128×128) to
                # the KV grid resolution (num_patches_kv^0.5 × num_patches_kv^0.5).
                n_kv = self.fusion.num_patches_kv
                kv_h = kv_w = int(n_kv ** 0.5)
                oe = olmoearth.permute(0, 3, 1, 2)           # (B, 768, H_emb, W_emb)
                oe = F.adaptive_avg_pool2d(oe, (kv_h, kv_w))  # (B, 768, kv_h, kv_w)
                oe = oe.permute(0, 2, 3, 1)                  # (B, kv_h, kv_w, 768)
                context = self.embed_head(oe)                 # (B, kv_h, kv_w, 1024)
...
class FusionBackboneMmsegV3(BaseModule):
    """Fusion backbone v3 — loads real olmoearth embeddings from .npy files.

    v3 changes vs v2:
      - olmoearth receives a (B, H_emb, W_emb, 768) tensor pre-loaded from
        .npy embedding files (loaded by LoadOlmoEarthEmbedding transform).
      - Embeds are spatially pooled to the KV resolution, projected through
        embed_head, and fed as context to the fusion cross-attention.
      - When olmoearth=None, falls back to the learnable fusion_mask_token.
...
class OlmoEarthEncoderDecoder(EncoderDecoder):
    """EncoderDecoder wrapper that passes olmoearth embeddings to the backbone.

    Expects each data_sample.metainfo to contain an 'olmoearth_embedding'
    tensor (B, H_emb, W_emb, 768), loaded by the LoadOlmoEarthEmbedding
    transform and carried through PackSegInputs with meta_keys=['olmoearth_embedding'].
    """

    @staticmethod
    def _collate_olmoearth(batch_metas):
        """batch_metas: list of metainfo dicts, each may have 'olmoearth_embedding'."""
        embeds = []
        for meta in batch_metas:
            emb = meta.get('olmoearth_embedding')
            if emb is not None:
                embeds.append(emb if isinstance(emb, torch.Tensor) else torch.from_numpy(emb))
        if not embeds:
            return None
        return torch.stack(embeds, dim=0)

    def extract_feat(self, inputs, olmoearth=None):
        return self.backbone(inputs, olmoearth=olmoearth)

    def loss(self, inputs, data_samples):
        batch_img_metas = [ds.metainfo for ds in data_samples]
        olmoearth = self._collate_olmoearth(batch_img_metas)
        if olmoearth is not None:
            olmoearth = olmoearth.to(inputs.device)
        x = self.extract_feat(inputs, olmoearth)
        losses = dict()
