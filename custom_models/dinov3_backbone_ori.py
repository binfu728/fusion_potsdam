"""纯 DINOv3 backbone + DINOv3_Adapter → mmseg。

兼容两种 checkpoint 格式：
  - 官方 DINOv3 预训练（LVD-1689M / SAT-493M）：bare keys（无前缀）
  - fusion checkpoint：keys 带 ``backbone.`` 前缀
backbone 冻结，只训 adapter (SPM+InteractionBlocks) + m2f head。
"""
import sys as _sys
...
class DINOv3BackboneMmseg(BaseModule):
    ...
    def _load_backbone_checkpoint(self, checkpoint):
        """加载 ViT 权重，同时兼容两种 checkpoint 格式：
        1. fusion checkpoint: keys 带 ``backbone.`` 前缀 → 去前缀
        2. 官方 DINOv3 checkpoint: bare keys（无前缀）→ 直接加载
        """
        ...
        # 先尝试 backbone.* 前缀（fusion checkpoint 格式）
        sd = {}
        for k, v in ckpt.items():
            if isinstance(v, torch.Tensor) and k.startswith("backbone."):
                sd[k[len("backbone."):]] = v

        # 如果没有找到 backbone.* 键，认为是官方 DINOv3 格式（bare keys）
        if not sd:
            for k, v in ckpt.items():
