...
# ═══════════════════════════════════════════════════════════════════════════
# 2. Stub OlmoEarth (produces context matching organize_embed_v2 output)
# ═══════════════════════════════════════════════════════════════════════════

class StubOlmoEarth(nn.Module):
    """Lightweight stand-in that returns a context tensor with the same shape
    as the real olmoearth → organize_embed_v2 pipeline: [B, H, W, 768]."""

    def __init__(self, embed_dim=768):
        super().__init__()
        self.embed_dim = embed_dim

    def forward(self, *, h=12, w=12, b=1):
        return torch.randn(b, h, w, self.embed_dim)


# ═══════════════════════════════════════════════════════════════════════════
# 3. UPHead (minimal reimplementation — avoids RS_vision_transformer dep)
# ═══════════════════════════════════════════════════════════════════════════

class UPHead(nn.Module):
    """Project olmoearth embedding → HR ViT embedding dim.

    NOTE: matches the released fusion checkpoint which stores only a Linear
    (no LayerNorm). load_checkpoint_forward.py strips LayerNorm at inference,
    and the checkpoint only has ``embed_head.decoder.0.*`` keys.
    """
...
class FusionBackboneWrapper(nn.Module):
    ...
    def __init__(
        ...
        self.olmoearth = StubOlmoEarth(embed_dim=olmoearth_embed)
        ...
        *,
        ...
        if has_fusion:
            ...
            if olmoearth is not None:
                context = self.embed_head(olmoearth)          # [B,H,W,384]
                context = rearrange(context, "b h w d -> b (h w) d")
            else:
                # olmoearth=None (mmseg calls backbone(x) with image only):
                # inject DUMMY MS/SAR context via the stub olmoearth, so the
                # fusion cross-attention runs with dummy tokens.
                n_kv = self.fusion.fusion_mask_token.shape[1]
                kv_h = int(n_kv ** 0.5)
                dummy = self.olmoearth(b=B, h=kv_h, w=kv_h).to(g_patch.device)
                context = self.embed_head(dummy)
                context = rearrange(context, "b h w d -> b (h w) d")
...
class FusionBackboneMmseg(BaseModule):
    """Fusion backbone registered as an mmseg MODEL.
